...
import * as graph from './view/graph.js';
...
const gcardHeight = familyCard.node().getBBox().height;
const gcardWidth = familyCard.node().getBBox().width;
familyCard.remove();

// parameter to automatically zoom;
let scaleWidth = width ;
let scaleHeight = height ;
let minNodeWidth = 0 ;
let minNodeHeight = 0 ;

// create the graph with the position for each node
graph.graph(nodes,gcardWidth,gcardHeight);
...
.force("x", d3.forceX())
.force("y", d3.forceY())
.force("collide", d3.forceCollide(gcardWidth))
.on("tick", tick);
